...
def extract_cars_from_page(page) -> list[dict]:
    ...
    for a in anchors:
        ...
        # Extract mileage e.g. "112,000 km" or "45,000 miles"
        m = re.search(r'([\d,]+)\s*(km|miles|mi)\b', card_text, re.IGNORECASE)
        car["mileage"] = f"{m.group(1)} {m.group(2).lower()}" if m else ""

        # Extract colour — look for common colours in card text
        colours = ["Black","White","Silver","Grey","Gray","Blue","Red","Green",
                   "Orange","Yellow","Brown","Beige","Purple","Gold","Bronze"]
        car["colour"] = next((c for c in colours if re.search(rf'\b{c}\b', card_text, re.IGNORECASE)), "")
